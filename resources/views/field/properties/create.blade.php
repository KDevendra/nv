...
<div class="max-w-4xl mx-auto">
    ...
    @if($errors->any())
        @php
            ...
            foreach ($sectionFields as $secName => $fields) {
                foreach ($fields as $f) {
                    if ($errors->has($f)) {
                        $errorsBySec[$secName][] = $errors->first($f);
                    }
                }
            }
            $uncategorised = [];
            ...
            foreach ($errors->keys() as $k) {
                if (!in_array($k, $allCatFields) && !str_starts_with($k, 'photos')) {
    ...
    <form method="POST" action="{{ route('field.properties.store') }}" enctype="multipart/form-data" x-data="{ isDraft: false }">
        @csrf
