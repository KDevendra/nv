...
@php
    ...
    if (!isset($correctFields)) { $correctFields = []; }
    ...
    $firstErrStep = -1;
    foreach ($stepErrCounts as $i => $c) { if ($c > 0) { $firstErrStep = $i; break; } }
...
<div id="wizard-progress" class="sticky top-0 z-30 bg-white border-b border-gray-200 shadow-sm -mx-1 px-3 py-2 mb-4">
    ...
    <div class="hidden sm:flex items-center gap-1">
        @php
            ...
            $stepTitles  = [
                'Location','Legal','Dimensions','Docks','Facilities',
                'Loading','Utilities','Financial','Surroundings','Emergency','Photos','Remarks'
            ];
        @endphp
        @foreach($stepLetters as $i => $ltr)
        <div class="flex-1 flex items-center">
            <button type="button"
                onclick="wizardGoTo({{ $i }})"
                id="wiz-dot-{{ $i }}"
                title="{{ $stepLetters[$i] }}. {{ $stepTitles[$i] }}"
                class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all flex-shrink-0
                       border-2 border-transparent wiz-dot"
                data-step="{{ $i }}">
                {{-- Error count badge shown if step has errors --}}
                @if($stepErrCounts[$i] > 0)
                    <span class="relative">{{ $ltr }}<span class="absolute -top-1 -right-2 w-3 h-3 bg-red-500 rounded-full text-[8px] text-white flex items-center justify-center">!</span></span>
                @else
                    {{ $ltr }}
                @endif
            </button>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        ...
        @if($sec_errs('B. Legal & Statutory Compliance') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('B. Legal & Statutory Compliance') }} error(s)</span>
        @endif
    </div>
    <div class="{{ $sb }}">
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <h3 class="text-sm font-semibold text-zendo-navy" data-section-title="D. Dock, Exit &amp; Width Details">D. Dock, Exit &amp; Width Details</h3>
        @if($sec_errs('D. Dock, Exit & Width Details') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('D. Dock, Exit & Width Details') }} error(s)</span>
        @endif
    </div>
    <div class="px-5 py-5 space-y-6">
        ...
        <div x-data="{
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-4">
                ...
                <div>
                    ...
                    <input type="number" min="0" name="dock_front" x-model.number="dock_front" value="{{ $v('dock_front') }}" {{ $req('dock_front') }} class="{{ $ic }}">
                ...
                <div>
                    ...
                    <input type="number" min="0" name="dock_left" x-model.number="dock_left" value="{{ $v('dock_left') }}" {{ $req('dock_left') }} class="{{ $ic }}">
                ...
                <div>
                    ...
                    <input type="number" min="0" name="dock_right" x-model.number="dock_right" value="{{ $v('dock_right') }}" {{ $req('dock_right') }} class="{{ $ic }}">
                ...
                <div>
                    ...
                    <input type="number" min="0" name="dock_back" x-model.number="dock_back" value="{{ $v('dock_back') }}" {{ $req('dock_back') }} class="{{ $ic }}">
                    @error('dock_back')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                @endif
                @if($fc('dock_door_count')->keep_field)
                <div>
                    <label class="{{ $lc }} flex items-center gap-1">Total <span class="text-[10px] text-gray-400 font-normal">(auto)</span> {!! $ast('dock_door_count') !!}</label>
                    <div class="relative">
        ...
        <div x-data="{
            ...
            <div class="flex items-center gap-4 mb-4">
                ...
                <div class="flex rounded-lg border border-gray-300 overflow-hidden text-xs font-semibold">
                    <label class="relative cursor-pointer">
                        <input type="radio" name="has_dock_leveller" value="1" x-model.number="hasLev" @change="hasLev=true"
                            {{ old('has_dock_leveller', $entry?->has_dock_leveller) == '1' ? 'checked' : '' }} class="sr-only">
                    ...
                    <label class="relative cursor-pointer">
                        <input type="radio" name="has_dock_leveller" value="0" @change="setNo()"
                            {{ old('has_dock_leveller', $entry?->has_dock_leveller) === false || old('has_dock_leveller', $entry?->has_dock_leveller) == '0' ? 'checked' : '' }} class="sr-only">
        ...
        <div x-data="{
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-4">
                ...
                <div><label class="{{ $lc }}">Front {!! $ast('fire_exit_front') !!}</label><input type="number" min="0" name="fire_exit_front" x-model.number="fe_front" value="{{ $v('fire_exit_front') }}" {{ $req('fire_exit_front') }} class="{{ $ic }}">@error('fire_exit_front')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                @endif
                @if($fc('fire_exit_left')->keep_field)
                <div><label class="{{ $lc }}">Left {!! $ast('fire_exit_left') !!}</label><input type="number" min="0" name="fire_exit_left" x-model.number="fe_left" value="{{ $v('fire_exit_left') }}" {{ $req('fire_exit_left') }} class="{{ $ic }}">@error('fire_exit_left')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                @endif
                @if($fc('fire_exit_right')->keep_field)
                <div><label class="{{ $lc }}">Right {!! $ast('fire_exit_right') !!}</label><input type="number" min="0" name="fire_exit_right" x-model.number="fe_right" value="{{ $v('fire_exit_right') }}" {{ $req('fire_exit_right') }} class="{{ $ic }}">@error('fire_exit_right')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                @endif
                @if($fc('fire_exit_back')->keep_field)
                <div><label class="{{ $lc }}">Back {!! $ast('fire_exit_back') !!}</label><input type="number" min="0" name="fire_exit_back" x-model.number="fe_back" value="{{ $v('fire_exit_back') }}" {{ $req('fire_exit_back') }} class="{{ $ic }}">@error('fire_exit_back')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror</div>
                @endif
                <div>
                    <label class="{{ $lc }} flex items-center gap-1">Total <span class="text-[10px] text-gray-400 font-normal">(auto)</span></label>
                    <div class="relative"><input type="number" :value="total" readonly class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-gray-50 text-gray-700 cursor-not-allowed font-semibold"><span class="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-gray-400">= F+L+R+B</span></div>
        ...
        <div>
            ...
            <div class="space-y-3">
                ...
                @php $lf = 'canopy_length_'.$side; $wf = 'canopy_width_'.$side; @endphp
                @if($fc($wf)->keep_field)
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <h3 class="text-sm font-semibold text-zendo-navy" data-section-title="E. Facility Details">E. Facility Details</h3>
        @if($sec_errs('E. Facility Details') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('E. Facility Details') }} error(s)</span>
        @endif
    </div>
    <div class="{{ $sb }}">
        ...
        <div class="sm:col-span-2 lg:col-span-3" x-data="officeWidget">
            <div class="flex items-center gap-4 mb-3">
                ...
                <div class="flex rounded-lg border border-gray-300 overflow-hidden text-xs font-semibold">
                    <label class="relative cursor-pointer">
                        <input type="radio" name="has_offices" value="1" @change="hasOffices=true"
                            {{ $initHasOffices === 'true' ? 'checked' : '' }} class="sr-only">
                        <span class="block px-5 py-2 transition-colors border-r border-gray-300 select-none"
                              :class="hasOffices===true ? 'bg-emerald-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'">Yes</span>
                    </label>
                    <label class="relative cursor-pointer">
                        <input type="radio" name="has_offices" value="0" @change="setNo()"
                            {{ $initHasOffices === 'false' ? 'checked' : '' }} class="sr-only">
                        <span class="block px-5 py-2 transition-colors select-none"
                              :class="hasOffices===false ? 'bg-red-500 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'">No</span>
                    </label>
                </div>
            ...
            <div x-show="hasOffices===true" x-cloak class="space-y-3">
                <template x-for="(office, i) in offices" :key="i">
                    <div class="flex items-end gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100">
                        ...
                        <div class="flex-1">
                            <label class="block text-xs font-medium text-gray-600 mb-1">Length (L) <span x-show="i===0" class="text-red-500">*</span></label>
                            <input type="number" step="0.01" min="0" x-model="office.l"
                                :required="i===0 && !$el.form.noValidate" placeholder="L (ft)" class="{{ $ic }} text-sm">
                        </div>
                        <div class="flex-1">
                            <label class="block text-xs font-medium text-gray-600 mb-1">Width (W) <span x-show="i===0" class="text-red-500">*</span></label>
                            <input type="number" step="0.01" min="0" x-model="office.w"
                                :required="i===0 && !$el.form.noValidate" placeholder="W (ft)" class="{{ $ic }} text-sm">
                        </div>
                        <div class="w-24 flex-shrink-0">
                            ...
                            <div class="px-2 py-2 bg-white border border-gray-200 rounded-lg text-xs font-semibold text-gray-600 text-center"
                                 x-text="(office.l&&office.w)?(parseFloat(office.l)*parseFloat(office.w)).toFixed(1)+' sq ft':'—'"></div>
                        </div>
            ...
            <div x-show="hasOffices===false" x-cloak class="text-xs text-gray-400 italic px-1 mt-1">No offices — count and sizes set to zero.</div>
        ...
        <div><label class="{{ $lc }}">Canteen {!! $ast('canteen') !!}</label>
            ...
            @error('canteen')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('canteen') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Canteen Size {!! $ast('canteen_size') !!}</label>
            ...
            @error('canteen_size')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('canteen_size') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">STP Plant {!! $ast('stp_plant') !!}</label>
            ...
            @error('stp_plant')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('stp_plant') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">STP Capacity {!! $ast('stp_capacity') !!}</label>
            ...
            @error('stp_capacity')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('stp_capacity') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">No. of Washrooms {!! $ast('washrooms') !!}</label>
            ...
            @error('washrooms')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('washrooms') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">No. of Urinals {!! $ast('no_of_urinals') !!}</label>
            ...
            @error('no_of_urinals')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('no_of_urinals') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">No. of Closets {!! $ast('no_of_closets') !!}</label>
            ...
            @error('no_of_closets')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('no_of_closets') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">{{ $flbl }} {!! $ast($fk) !!}</label>
            ...
            @error($fk)<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk($fk) !!}
        </div>
        @endif
        @endforeach
        ...
        <div><label class="{{ $lc }}">Mezzanine Size {!! $ast('mezzanine_size') !!}</label>
            ...
            @error('mezzanine_size')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('mezzanine_size') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Structure Type {!! $ast('structure_type') !!}</label>
            ...
            @error('structure_type')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('structure_type') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Flooring Type {!! $ast('flooring_type') !!}</label>
            ...
            @error('flooring_type')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('flooring_type') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Ventilation &amp; Lighting {!! $ast('ventilation_lighting') !!}</label>
            ...
            @error('ventilation_lighting')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('ventilation_lighting') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Roof Insulation {!! $ast('insulation_roof') !!}</label>
            ...
            @error('insulation_roof')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('insulation_roof') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Side Insulation {!! $ast('insulation_side') !!}</label>
            ...
            @error('insulation_side')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('insulation_side') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Fire Sprinkler {!! $ast('fire_sprinkler') !!}</label>
            ...
            @error('fire_sprinkler')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('fire_sprinkler') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">No. of Companies in Same Premise {!! $ast('no_of_companies_same_premise') !!}</label>
            ...
            @error('no_of_companies_same_premise')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('no_of_companies_same_premise') !!}
        </div>
        @endif
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <h3 class="text-sm font-semibold text-zendo-navy" data-section-title="F. Loading &amp; Docking">F. Loading &amp; Docking Facilities</h3>
        @if($sec_errs('F. Loading & Docking') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('F. Loading & Docking') }} error(s)</span>
        @endif
    </div>
    <div class="{{ $sb }}">
        ...
        <div><label class="{{ $lc }}">Dock Type {!! $ast('dock_type') !!}</label>
            ...
            @error('dock_type')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('dock_type') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Dock Height (ft) {!! $ast('dock_height') !!}</label>
            ...
            @error('dock_height')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('dock_height') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Truck Movement {!! $ast('truck_movement') !!}</label>
            ...
            @error('truck_movement')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('truck_movement') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Office / Cabin Area (sq ft) {!! $ast('office_cabin_area') !!}</label>
            ...
            @error('office_cabin_area')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('office_cabin_area') !!}
        </div>
        @endif
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <h3 class="text-sm font-semibold text-zendo-navy" data-section-title="G. Utilities &amp; Infrastructure">G. Utilities &amp; Infrastructure</h3>
        @if($sec_errs('G. Utilities & Infrastructure') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('G. Utilities & Infrastructure') }} error(s)</span>
        @endif
    </div>
    <div class="{{ $sb }}">
        ...
        <div><label class="{{ $lc }}">Power Sanctioned (KVA) {!! $ast('power_sanctioned_kva') !!}</label>
            ...
            @error('power_sanctioned_kva')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('power_sanctioned_kva') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">DISCOM Name {!! $ast('discom_name') !!}</label>
            ...
            @error('discom_name')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('discom_name') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Water Source {!! $ast('water_source') !!}</label>
            ...
            @error('water_source')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('water_source') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Water Tank Capacity {!! $ast('water_tank_capacity') !!}</label>
            ...
            @error('water_tank_capacity')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('water_tank_capacity') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Fire Fighting System {!! $ast('fire_fighting_system') !!}</label>
            ...
            @error('fire_fighting_system')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('fire_fighting_system') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Solar {!! $ast('solar') !!}</label>
            ...
            @error('solar')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('solar') !!}
        </div>
        @endif
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <h3 class="text-sm font-semibold text-zendo-navy" data-section-title="H. Financial &amp; Lease Terms">H. Financial &amp; Lease Terms</h3>
        @if($sec_errs('H. Financial & Lease Terms') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('H. Financial & Lease Terms') }} error(s)</span>
        @endif
    </div>
    <div class="{{ $sb }}">
        ...
        <div><label class="{{ $lc }}">Lease / Sale Status {!! $ast('deal_type') !!}</label>
            ...
            @error('deal_type')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('deal_type') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Expected Rent (₹/sq ft/month) {!! $ast('expected_rent') !!}</label>
            ...
            @error('expected_rent')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('expected_rent') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Expected Sale Price (₹) {!! $ast('expected_sale_price') !!}</label>
            ...
            @error('expected_sale_price')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('expected_sale_price') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Security Deposit (months) {!! $ast('security_deposit_months') !!}</label>
            ...
            @error('security_deposit_months')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('security_deposit_months') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Lock-in Period (years) {!! $ast('lock_in_years') !!}</label>
            ...
            @error('lock_in_years')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('lock_in_years') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Available From Date {!! $ast('available_from') !!}</label>
            <input type="date" name="available_from" {{ $req('available_from') }}
                ...
                class="{{ $ic }}">
            @error('available_from')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('available_from') !!}
        </div>
        @endif
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <h3 class="text-sm font-semibold text-zendo-navy" data-section-title="I. Surroundings &amp; Environment">I. Surroundings &amp; Environment</h3>
        @if($sec_errs('I. Surroundings & Environment') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('I. Surroundings & Environment') }} error(s)</span>
        @endif
    </div>
    <div class="{{ $sb }}">
        ...
        <div><label class="{{ $lc }}">Approach Road Width (ft) {!! $ast('approach_road_width') !!}</label>
            ...
            @error('approach_road_width')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('approach_road_width') !!}
        </div>
        @endif
        ...
        <div class="sm:col-span-2"><label class="{{ $lc }}">Top Neighbouring Companies {!! $ast('top_neighbouring_companies') !!}</label>
            ...
            @error('top_neighbouring_companies')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('top_neighbouring_companies') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Flood / Water-Logging Risk {!! $ast('flood_risk') !!}</label>
            ...
            @error('flood_risk')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('flood_risk') !!}
        </div>
        @endif
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <h3 class="text-sm font-semibold text-zendo-navy" data-section-title="J. Health &amp; Emergency Nearby">J. Health &amp; Emergency Facilities Nearby</h3>
        @if($sec_errs('J. Health & Emergency Nearby') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('J. Health & Emergency Nearby') }} error(s)</span>
        @endif
    </div>
    <div class="{{ $sb }}">
        ...
        <div><label class="{{ $lc }}">Nearest Hospital (km) {!! $ast('nearest_hospital_km') !!}</label>
            ...
            @error('nearest_hospital_km')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('nearest_hospital_km') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Nearest Fire Station (km) {!! $ast('nearest_fire_station_km') !!}</label>
            ...
            @error('nearest_fire_station_km')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('nearest_fire_station_km') !!}
        </div>
        @endif
        ...
        <div><label class="{{ $lc }}">Nearest Police Station (km) {!! $ast('nearest_police_station_km') !!}</label>
            ...
            @error('nearest_police_station_km')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror {!! $rmk('nearest_police_station_km') !!}
        </div>
        @endif
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        ...
        @if($sec_errs('K. Photographs') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('K. Photographs') }} error(s)</span>
        @endif
    </div>
        ...
        @foreach($slots as $index => $slotLabel)
            ...
            <div class="flex flex-col items-center gap-2">
                <div class="relative w-full aspect-square rounded-xl overflow-hidden border-2 border-dashed border-gray-200 bg-gray-50 group cursor-pointer"
                     ...
                     @if(!$photoLocked) onclick="openCamera({{ $index }}, '{{ addslashes($slotLabel) }}')" @endif>
                ...
                <input type="file" name="photos[{{ $index }}]" id="photo-{{ $index }}" accept="image/*" class="sr-only" @if($photoLocked) disabled @endif>
                @if($photoLocked)
...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-4">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <h3 class="text-sm font-semibold text-zendo-navy" data-section-title="L. General Remarks">L. General Remarks &amp; Field Observations</h3>
        @if($sec_errs('L. General Remarks') > 0)
            <span class="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{{ $sec_errs('L. General Remarks') }} error(s)</span>
        @endif
    </div>
    <div class="{{ $sb }}">
...
{{-- ═══════════════════════════════════════════════════════
     ...
     ═══════════════════════════════════════════════════════ --}}
<div id="wizard-nav" class="sticky bottom-0 left-0 right-0 z-30 bg-white border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.06)] px-4 py-3 mt-4 -mx-1">
