...
@section('content')
    @php
        ...
        // config('property_entry_sections.factory_manufacturing_industrial').
        // Letters skip (no F or I here) because the spec sheet itself skips them;
        // each tab must correspond to exactly one wizard-step-content panel below,
        // in the same order, since they're matched by position, not by key.
        $steps = [
            ['key' => 'A', 'title' => 'Submitter & Owner Details'],
            ['key' => 'B', 'title' => 'Location & Identification'],
            ['key' => 'C', 'title' => 'Unit Specifications'],
            ['key' => 'D', 'title' => 'Legal & Statutory Compliance'],
            ['key' => 'E', 'title' => 'Utilities & Infrastructure'],
    ...
    <div class="max-w-5xl mx-auto space-y-6" x-data="stepWizard()">
        <div class="flex items-center justify-between mb-2">
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Complete section-by-section submission form for Factory /
        ...
        <form method="POST"
            action="{{ isset($property) ? route('owner.properties.factory-manufacturing-industrial.update', $property) : route('owner.properties.factory-manufacturing-industrial.store') }}"
            ...
            <x-property-wizard-shell :steps="$steps" :title="'Factory / Manufacturing / Industrial Submission Form'"
                :propertyType="'factory_manufacturing_industrial'">
                ...
                <div class="wizard-step-content space-y-6" style="display:none">
                    ...
                    <div class="border-t pt-4 first:border-t-0 first:pt-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Plot Area (sq ft / acre) <span
                                        class="text-red-500">*</span></label>
                                <input required type="number" step="any" name="plot_area_sq_ft_acre"
                                    value="{{ old('plot_area_sq_ft_acre', $property?->fieldValue('plot_area_sq_ft_acre') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Covered / Built-up Area (sq ft)
                                    <span class="text-red-500">*</span></label>
                                <input required type="number" step="any" name="covered_built_up_area_sq_ft"
                                    value="{{ old('covered_built_up_area_sq_ft', $property?->fieldValue('covered_built_up_area_sq_ft') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Carpet / Production Area (sq ft)
                                </label>
                                <input type="number" step="any" name="carpet_production_area_sq_ft"
                                    value="{{ old('carpet_production_area_sq_ft', $property?->fieldValue('carpet_production_area_sq_ft') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">No. of Floors </label>
                                <input type="number" step="any" name="no_of_floors"
                            ...
                            <div>
                                ...
                                <select name="which_floor_if_floor"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                            ...
                            <div>
                                ...
                                <input type="text" name="clear_height_ft"
                                    value="{{ old('clear_height_ft', $property?->fieldValue('clear_height_ft') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Shed Type </label>
                                <select name="shed_type"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Crane / Gantry Provision
                                </label>
                                <select name="crane_gantry_provision"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    ...
                                    <option value="No" {{ old('crane_gantry_provision', $property?->fieldValue('crane_gantry_provision') ?? '') === 'No' ? 'selected' : '' }}>
                                        No</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Office / Admin Block Area (sq
                                    ft) </label>
                                <input type="number" step="any" name="office_admin_block_area_sq_ft"
                                    value="{{ old('office_admin_block_area_sq_ft', $property?->fieldValue('office_admin_block_area_sq_ft') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Plant & Machinery Included <span
                                        class="text-red-500">*</span></label>
                                <select required name="plant_machinery_included"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Furnishing / Readiness <span
                                        class="text-red-500">*</span></label>
                                <select required name="furnishing_readiness"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select required name="ownership_title_type"
                                    ...
                                    <option value="GIDC" {{ old('ownership_title_type', $property?->fieldValue('ownership_title_type') ?? '') === 'GIDC' ? 'selected' : '' }}>
                                        GIDC</option>
                                    <option value="etc" {{ old('ownership_title_type', $property?->fieldValue('ownership_title_type') ?? '') === 'etc' ? 'selected' : '' }}>
                                        etc</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">CLU / Industrial Use Approved
                                    <span class="text-red-500">*</span></label>
                                <select required name="clu_industrial_use_approved"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                            ...
                            <div>
                                ...
                                <select name="factory_licence"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Held" {{ old('factory_licence', $property?->fieldValue('factory_licence') ?? '') === 'Held' ? 'selected' : '' }}>Held</option>
                                    <option value="Applied" {{ old('factory_licence', $property?->fieldValue('factory_licence') ?? '') === 'Applied' ? 'selected' : '' }}>
                                        Applied</option>
                                    <option value="None" {{ old('factory_licence', $property?->fieldValue('factory_licence') ?? '') === 'None' ? 'selected' : '' }}>None</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Pollution / Consent to Operate
                                    <span class="text-red-500">*</span></label>
                                <select required name="pollution_consent_to_operate"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                            ...
                            <div>
                                ...
                                <select name="pollution_category"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Green" {{ old('pollution_category', $property?->fieldValue('pollution_category') ?? '') === 'Green' ? 'selected' : '' }}>
                                        Green</option>
                                    <option value="Orange" {{ old('pollution_category', $property?->fieldValue('pollution_category') ?? '') === 'Orange' ? 'selected' : '' }}>
                                        Orange</option>
                                    <option value="Red" {{ old('pollution_category', $property?->fieldValue('pollution_category') ?? '') === 'Red' ? 'selected' : '' }}>Red
                                    </option>
                                    <option value="White" {{ old('pollution_category', $property?->fieldValue('pollution_category') ?? '') === 'White' ? 'selected' : '' }}>
                                        White</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Fire NOC <span
                                        class="text-red-500">*</span></label>
                                <select required name="fire_noc"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Environment Clearance (if
                                    applicable) </label>
                                <select name="environment_clearance_if_applicable"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Encumbrance / Loan <span
                                        class="text-red-500">*</span></label>
                                <select required name="encumbrance_loan"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="None" {{ old('encumbrance_loan', $property?->fieldValue('encumbrance_loan') ?? '') === 'None' ? 'selected' : '' }}>None
                                    </option>
                                    <option value="Loan" {{ old('encumbrance_loan', $property?->fieldValue('encumbrance_loan') ?? '') === 'Loan' ? 'selected' : '' }}>Loan
                                    </option>
                                    <option value="Mortgage" {{ old('encumbrance_loan', $property?->fieldValue('encumbrance_loan') ?? '') === 'Mortgage' ? 'selected' : '' }}>
                                        Mortgage</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Sanctioned Power Load (KW/KVA)
                                    <span class="text-red-500">*</span></label>
                                <input required type="text" name="sanctioned_power_load_kw_kva"
                            ...
                            <div>
                                ...
                                <select name="power_connection_type"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="HT" {{ old('power_connection_type', $property?->fieldValue('power_connection_type') ?? '') === 'HT' ? 'selected' : '' }}>
                                        HT</option>
                                    <option value="LT" {{ old('power_connection_type', $property?->fieldValue('power_connection_type') ?? '') === 'LT' ? 'selected' : '' }}>
                                        LT</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">DISCOM Name </label>
                                <input type="text" name="discom_name"
                                    value="{{ old('discom_name', $property?->fieldValue('discom_name') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">DG Set / Backup </label>
                                <select name="dg_set_backup"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Yes (KVA)" {{ old('dg_set_backup', $property?->fieldValue('dg_set_backup') ?? '') === 'Yes (KVA)' ? 'selected' : '' }}>
                                        Yes (KVA)</option>
                            ...
                            <div>
                                ...
                                <input type="text" name="water_source_capacity"
                                    value="{{ old('water_source_capacity', $property?->fieldValue('water_source_capacity') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Effluent Treatment (ETP/STP)
                                </label>
                                <select name="effluent_treatment_etp_stp"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Boiler / Steam / Gas Line
                                </label>
                                <select name="boiler_steam_gas_line"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Yes" {{ old('boiler_steam_gas_line', $property?->fieldValue('boiler_steam_gas_line') ?? '') === 'Yes' ? 'selected' : '' }}>
                                        Yes</option>
                                    <option value="No" {{ old('boiler_steam_gas_line', $property?->fieldValue('boiler_steam_gas_line') ?? '') === 'No' ? 'selected' : '' }}>
                                        No</option>
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Truck Movement / Container
                                    Access </label>
                                <select name="truck_movement_container_access"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
