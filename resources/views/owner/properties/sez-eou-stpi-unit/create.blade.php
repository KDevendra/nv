...
@section('content')
    @php
        ...
        // config('property_entry_sections.sez_eou_stpi_unit'). Letters skip (no F
        // or I here) because the spec sheet itself skips them; each tab must
        // correspond to exactly one wizard-step-content panel below, in the same
        // order, since they're matched by position, not by key.
        $steps = [
            ['key' => 'A', 'title' => 'Submitter & Owner Details'],
            ['key' => 'B', 'title' => 'Location & Identification'],
            ['key' => 'C', 'title' => 'Unit Configuration'],
            ['key' => 'D', 'title' => 'SEZ / Export Compliance'],
            ['key' => 'E', 'title' => 'Infrastructure'],
    ...
    <div class="max-w-5xl mx-auto space-y-6" x-data="stepWizard()">
        <div class="flex items-center justify-between mb-2">
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Complete section-by-section submission form for SEZ / EOU / STPI Unit
                </p>
            </div>
        ...
        <form method="POST"
            action="{{ isset($property) ? route('owner.properties.sez-eou-stpi-unit.update', $property) : route('owner.properties.sez-eou-stpi-unit.store') }}"
            ...
            <x-property-wizard-shell :steps="$steps" :title="'SEZ / EOU / STPI Unit Submission Form'"
                :propertyType="'sez_eou_stpi_unit'">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select required name="property_type"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="SEZ Unit" {{ old('property_type', $property?->fieldValue('property_type') ?? '') === 'SEZ Unit' ? 'selected' : '' }}>SEZ Unit</option>
                                    <option value="EOU" {{ old('property_type', $property?->fieldValue('property_type') ?? '') === 'EOU' ? 'selected' : '' }}>EOU</option>
                                    <option value="STPI Unit" {{ old('property_type', $property?->fieldValue('property_type') ?? '') === 'STPI Unit' ? 'selected' : '' }}>
                                        STPI Unit</option>
                                    <option value="FTWZ Unit" {{ old('property_type', $property?->fieldValue('property_type') ?? '') === 'FTWZ Unit' ? 'selected' : '' }}>
                                        FTWZ Unit</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Zone / Park Name <span
                                        class="text-red-500">*</span></label>
                                <input required type="text" name="zone_park_name"
                                    value="{{ old('zone_park_name', $property?->fieldValue('zone_park_name') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Space Type <span
                                        class="text-red-500">*</span></label>
                                <select required name="space_type"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Built-up office" {{ old('space_type', $property?->fieldValue('space_type') ?? '') === 'Built-up office' ? 'selected' : '' }}>Built-up office</option>
                                    <option value="Built-up factory" {{ old('space_type', $property?->fieldValue('space_type') ?? '') === 'Built-up factory' ? 'selected' : '' }}>Built-up factory</option>
                                    <option value="Bare shell" {{ old('space_type', $property?->fieldValue('space_type') ?? '') === 'Bare shell' ? 'selected' : '' }}>Bare shell</option>
                                    <option value="Land for own build" {{ old('space_type', $property?->fieldValue('space_type') ?? '') === 'Land for own build' ? 'selected' : '' }}>Land for own build</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Carpet / Usable Area (sq ft)
                                    <span class="text-red-500">*</span></label>
                                <input required type="number" step="any" name="carpet_usable_area_sq_ft"
                                    value="{{ old('carpet_usable_area_sq_ft', $property?->fieldValue('carpet_usable_area_sq_ft') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="text" name="floor_total_floors"
                                    value="{{ old('floor_total_floors', $property?->fieldValue('floor_total_floors') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select name="sector_focus"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="IT" {{ old('sector_focus', $property?->fieldValue('sector_focus') ?? '') === 'IT' ? 'selected' : '' }}>IT</option>
                                    <option value="ITES" {{ old('sector_focus', $property?->fieldValue('sector_focus') ?? '') === 'ITES' ? 'selected' : '' }}>ITES</option>
                                    <option value="Electronics" {{ old('sector_focus', $property?->fieldValue('sector_focus') ?? '') === 'Electronics' ? 'selected' : '' }}>
                                        Electronics</option>
                                    <option value="Pharma" {{ old('sector_focus', $property?->fieldValue('sector_focus') ?? '') === 'Pharma' ? 'selected' : '' }}>Pharma</option>
                                    <option value="Biotech" {{ old('sector_focus', $property?->fieldValue('sector_focus') ?? '') === 'Biotech' ? 'selected' : '' }}>Biotech</option>
                                    <option value="Engineering" {{ old('sector_focus', $property?->fieldValue('sector_focus') ?? '') === 'Engineering' ? 'selected' : '' }}>
                                        Engineering</option>
                                    <option value="Multi-product" {{ old('sector_focus', $property?->fieldValue('sector_focus') ?? '') === 'Multi-product' ? 'selected' : '' }}>Multi-product</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="age_of_building"
                                    ...
                                    <option value="10+ yrs" {{ old('age_of_building', $property?->fieldValue('age_of_building') ?? '') === '10+ yrs' ? 'selected' : '' }}>
                                        10+ yrs</option>
                                </select>
                            </div>
                        </div>
                ...
                <div class="wizard-step-content space-y-6" style="display:none">
                    ...
                    <div class="border-t pt-4 first:border-t-0 first:pt-0">
                        <div class="border-b pb-2 mb-4">
                            <h4 class="text-base font-bold text-zendo-navy">SECTION D — SEZ / EXPORT COMPLIANCE
                                (India-specific — key value-add)</h4>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">

                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Zone Notification Status <span
                                        class="text-red-500">*</span></label>
                                <select required name="zone_notification_status"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Notified Operational SEZ" {{ old('zone_notification_status', $property?->fieldValue('zone_notification_status') ?? '') === 'Notified Operational SEZ' ? 'selected' : '' }}>Notified Operational SEZ</option>
                                    <option value="STPI Notified" {{ old('zone_notification_status', $property?->fieldValue('zone_notification_status') ?? '') === 'STPI Notified' ? 'selected' : '' }}>STPI Notified</option>
                                    <option value="EOU Bonded" {{ old('zone_notification_status', $property?->fieldValue('zone_notification_status') ?? '') === 'EOU Bonded' ? 'selected' : '' }}>EOU Bonded</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Processing / Non-Processing Area
                                    <span class="text-red-500">*</span></label>
                                <select required name="processing_non_processing_area"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Processing Area" {{ old('processing_non_processing_area', $property?->fieldValue('processing_non_processing_area') ?? '') === 'Processing Area' ? 'selected' : '' }}>Processing Area</option>
                                    <option value="Non-Processing Area" {{ old('processing_non_processing_area', $property?->fieldValue('processing_non_processing_area') ?? '') === 'Non-Processing Area' ? 'selected' : '' }}>Non-Processing Area</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Developer Consent for Sub-lease
                                    <span class="text-red-500">*</span></label>
                                <select required name="developer_consent_for_sub_lease"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Available" {{ old('developer_consent_for_sub_lease', $property?->fieldValue('developer_consent_for_sub_lease') ?? '') === 'Available' ? 'selected' : '' }}>Available</option>
                                    <option value="To be obtained" {{ old('developer_consent_for_sub_lease', $property?->fieldValue('developer_consent_for_sub_lease') ?? '') === 'To be obtained' ? 'selected' : '' }}>To be obtained</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Letter of Approval (LoA) Support
                                </label>
                                <select name="letter_of_approval_loa_support"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Developer assists" {{ old('letter_of_approval_loa_support', $property?->fieldValue('letter_of_approval_loa_support') ?? '') === 'Developer assists' ? 'selected' : '' }}>Developer assists</option>
                                    <option value="Applicant arranges" {{ old('letter_of_approval_loa_support', $property?->fieldValue('letter_of_approval_loa_support') ?? '') === 'Applicant arranges' ? 'selected' : '' }}>Applicant arranges</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Customs Bonding Status </label>
                                <select name="customs_bonding_status"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Bonded" {{ old('customs_bonding_status', $property?->fieldValue('customs_bonding_status') ?? '') === 'Bonded' ? 'selected' : '' }}>Bonded</option>
                                    <option value="To be bonded" {{ old('customs_bonding_status', $property?->fieldValue('customs_bonding_status') ?? '') === 'To be bonded' ? 'selected' : '' }}>To be bonded</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Ready for Unit Approval
                                    Committee </label>
                                <select name="ready_for_unit_approval_committee"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Yes" {{ old('ready_for_unit_approval_committee', $property?->fieldValue('ready_for_unit_approval_committee') ?? '') === 'Yes' ? 'selected' : '' }}>Yes</option>
                                    <option value="No" {{ old('ready_for_unit_approval_committee', $property?->fieldValue('ready_for_unit_approval_committee') ?? '') === 'No' ? 'selected' : '' }}>No</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Duty-free Import Benefit
                                </label>
                                <select name="duty_free_import_benefit"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Applicable" {{ old('duty_free_import_benefit', $property?->fieldValue('duty_free_import_benefit') ?? '') === 'Applicable' ? 'selected' : '' }}>Applicable</option>
                                    <option value="Not applicable" {{ old('duty_free_import_benefit', $property?->fieldValue('duty_free_import_benefit') ?? '') === 'Not applicable' ? 'selected' : '' }}>Not applicable</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">GST / Zero-rated Supply Eligible
                                </label>
                                <select name="gst_zero_rated_supply_eligible"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Yes" {{ old('gst_zero_rated_supply_eligible', $property?->fieldValue('gst_zero_rated_supply_eligible') ?? '') === 'Yes' ? 'selected' : '' }}>Yes</option>
                                    <option value="No" {{ old('gst_zero_rated_supply_eligible', $property?->fieldValue('gst_zero_rated_supply_eligible') ?? '') === 'No' ? 'selected' : '' }}>No</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Title / Lease Type <span
                                        class="text-red-500">*</span></label>
                                <select required name="title_lease_type"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Developer Leasehold" {{ old('title_lease_type', $property?->fieldValue('title_lease_type') ?? '') === 'Developer Leasehold' ? 'selected' : '' }}>Developer Leasehold</option>
                                    <option value="Sub-lease" {{ old('title_lease_type', $property?->fieldValue('title_lease_type') ?? '') === 'Sub-lease' ? 'selected' : '' }}>Sub-lease</option>
                                    <option value="Co-developer" {{ old('title_lease_type', $property?->fieldValue('title_lease_type') ?? '') === 'Co-developer' ? 'selected' : '' }}>Co-developer</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-t pt-4 first:border-t-0 first:pt-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="power_sanctioned_kva"
                                    value="{{ old('power_sanctioned_kva', $property?->fieldValue('power_sanctioned_kva') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select name="redundant_power_dg"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="N+1" {{ old('redundant_power_dg', $property?->fieldValue('redundant_power_dg') ?? '') === 'N+1' ? 'selected' : '' }}>N+1
                                    </option>
                                    <option value="N" {{ old('redundant_power_dg', $property?->fieldValue('redundant_power_dg') ?? '') === 'N' ? 'selected' : '' }}>N
                                    </option>
                                    <option value="None" {{ old('redundant_power_dg', $property?->fieldValue('redundant_power_dg') ?? '') === 'None' ? 'selected' : '' }}>
                                        None</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="air_conditioning"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Central" {{ old('air_conditioning', $property?->fieldValue('air_conditioning') ?? '') === 'Central' ? 'selected' : '' }}>
                                        Central</option>
                                    <option value="VRV" {{ old('air_conditioning', $property?->fieldValue('air_conditioning') ?? '') === 'VRV' ? 'selected' : '' }}>VRV
                                    </option>
                                    <option value="None" {{ old('air_conditioning', $property?->fieldValue('air_conditioning') ?? '') === 'None' ? 'selected' : '' }}>None
                                    </option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Telecom / Fibre Redundancy
                                </label>
                                <select name="telecom_fibre_redundancy"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Dual" {{ old('telecom_fibre_redundancy', $property?->fieldValue('telecom_fibre_redundancy') ?? '') === 'Dual' ? 'selected' : '' }}>Dual</option>
                                    <option value="Single" {{ old('telecom_fibre_redundancy', $property?->fieldValue('telecom_fibre_redundancy') ?? '') === 'Single' ? 'selected' : '' }}>Single</option>
                                    <option value="None" {{ old('telecom_fibre_redundancy', $property?->fieldValue('telecom_fibre_redundancy') ?? '') === 'None' ? 'selected' : '' }}>None</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="fire_safety_system"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Sprinkler+Hydrant" {{ old('fire_safety_system', $property?->fieldValue('fire_safety_system') ?? '') === 'Sprinkler+Hydrant' ? 'selected' : '' }}>Sprinkler+Hydrant</option>
                                    <option value="Basic" {{ old('fire_safety_system', $property?->fieldValue('fire_safety_system') ?? '') === 'Basic' ? 'selected' : '' }}>
                                        Basic</option>
                                    <option value="None" {{ old('fire_safety_system', $property?->fieldValue('fire_safety_system') ?? '') === 'None' ? 'selected' : '' }}>
                                        None</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="common_facilities[]" multiple class="select2-multiple w-full">
                                    @php $sel = old('common_facilities', $property?->fieldValue('common_facilities') ?? []); @endphp
                                    <option value="Cafeteria" {{ in_array('Cafeteria', (array) $sel) ? 'selected' : '' }}>
                                        Cafeteria</option>
                                    <option value="Crèche" {{ in_array('Crèche', (array) $sel) ? 'selected' : '' }}>Crèche
                                    </option>
                                    <option value="Bank" {{ in_array('Bank', (array) $sel) ? 'selected' : '' }}>Bank</option>
                                    <option value="Transport" {{ in_array('Transport', (array) $sel) ? 'selected' : '' }}>
                                        Transport</option>
                                    <option value="Security" {{ in_array('Security', (array) $sel) ? 'selected' : '' }}>
                                        Security</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select name="price_on_request"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Yes" {{ old('price_on_request', $property?->fieldValue('price_on_request') ?? '') === 'Yes' ? 'selected' : '' }}>Yes
                                    </option>
                            ...
                            <div>
                                ...
                                <select name="security_deposit_months"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="3" {{ old('security_deposit_months', $property?->fieldValue('security_deposit_months') ?? '') === '3' ? 'selected' : '' }}>
                                        3</option>
                                    <option value="6" {{ old('security_deposit_months', $property?->fieldValue('security_deposit_months') ?? '') === '6' ? 'selected' : '' }}>
                                        6</option>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select name="tenant_type_public"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="MNC" {{ old('tenant_type_public', $property?->fieldValue('tenant_type_public') ?? '') === 'MNC' ? 'selected' : '' }}>MNC
                                    </option>
                                    <option value="Corporate" {{ old('tenant_type_public', $property?->fieldValue('tenant_type_public') ?? '') === 'Corporate' ? 'selected' : '' }}>Corporate</option>
                                    <option value="Bank" {{ old('tenant_type_public', $property?->fieldValue('tenant_type_public') ?? '') === 'Bank' ? 'selected' : '' }}>
                                        Bank</option>
