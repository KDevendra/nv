...
@section('content')
    @php
        ...
        // config('property_entry_sections.service_apartment_pg'). Letters skip
        // (no D, F, H or I here) because the spec sheet itself skips them; each
        ...
        $steps = [
            ...
            ['key' => 'C', 'title' => 'Unit & Occupancy'],
            ['key' => 'E', 'title' => 'Services & Amenities'],
    ...
    <div class="max-w-5xl mx-auto space-y-6" x-data="stepWizard()">
        <div class="flex items-center justify-between mb-2">
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Complete section-by-section submission form for Service Apartment /
                    Co-living / PG</p>
            </div>
        ...
        <form method="POST"
            action="{{ isset($property) ? route('owner.properties.service-apartment-pg.update', $property) : route('owner.properties.service-apartment-pg.store') }}"
            ...
            <x-property-wizard-shell :steps="$steps" :title="'Service Apartment / Co-living / PG Submission Form'"
                :propertyType="'service_apartment_pg'">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select required name="property_type"
                                    ...
                                    <option value="Co-living" {{ old('property_type', $property?->fieldValue('property_type') ?? '') === 'Co-living' ? 'selected' : '' }}>
                                        Co-living</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Room / Occupancy Type <span
                                        class="text-red-500">*</span></label>
                                <select required name="room_occupancy_type"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Private" {{ old('room_occupancy_type', $property?->fieldValue('room_occupancy_type') ?? '') === 'Private' ? 'selected' : '' }}>Private</option>
                                    <option value="Twin" {{ old('room_occupancy_type', $property?->fieldValue('room_occupancy_type') ?? '') === 'Twin' ? 'selected' : '' }}>
                                        Twin</option>
                                    <option value="Triple" {{ old('room_occupancy_type', $property?->fieldValue('room_occupancy_type') ?? '') === 'Triple' ? 'selected' : '' }}>Triple</option>
                                    <option value="Dorm" {{ old('room_occupancy_type', $property?->fieldValue('room_occupancy_type') ?? '') === 'Dorm' ? 'selected' : '' }}>
                                        Dorm</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Total Rooms / Beds Available
                                    <span class="text-red-500">*</span></label>
                                <input required type="number" step="any" name="total_rooms_beds_available"
                                    value="{{ old('total_rooms_beds_available', $property?->fieldValue('total_rooms_beds_available') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Carpet Area per Unit (sq ft)
                                </label>
                                <input type="number" step="any" name="carpet_area_per_unit_sq_ft"
                                    value="{{ old('carpet_area_per_unit_sq_ft', $property?->fieldValue('carpet_area_per_unit_sq_ft') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                ...
                                <select required name="furnishing_status"
                                    ...
                                    <option value="Semi" {{ old('furnishing_status', $property?->fieldValue('furnishing_status') ?? '') === 'Semi' ? 'selected' : '' }}>
                                        Semi</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="attached_bathroom"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Yes" {{ old('attached_bathroom', $property?->fieldValue('attached_bathroom') ?? '') === 'Yes' ? 'selected' : '' }}>Yes
                                    </option>
                                    <option value="Shared" {{ old('attached_bathroom', $property?->fieldValue('attached_bathroom') ?? '') === 'Shared' ? 'selected' : '' }}>
                                        Shared</option>
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select name="target_occupant"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Students" {{ old('target_occupant', $property?->fieldValue('target_occupant') ?? '') === 'Students' ? 'selected' : '' }}>
                                        Students</option>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select name="meals_included"
                                    ...
                                    <option value="Breakfast" {{ old('meals_included', $property?->fieldValue('meals_included') ?? '') === 'Breakfast' ? 'selected' : '' }}>
                                        Breakfast</option>
                                    <option value="All meals" {{ old('meals_included', $property?->fieldValue('meals_included') ?? '') === 'All meals' ? 'selected' : '' }}>
                                        All meals</option>
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select name="common_amenities[]" multiple class="select2-multiple w-full">
                                    ...
                                    <option value="Kitchen" {{ in_array('Kitchen', (array) $sel) ? 'selected' : '' }}>Kitchen
                                    </option>
                                    <option value="Lounge" {{ in_array('Lounge', (array) $sel) ? 'selected' : '' }}>Lounge
                                    </option>
                ...
                <div class="wizard-step-content space-y-6" style="display:none">
                    ...
                    <div class="border-t pt-4 first:border-t-0 first:pt-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Rent per Bed / Room (₹/month)
                                    <span class="text-red-500">*</span></label>
                                <input required type="number" step="any" name="rent_per_bed_room_month"
                                    value="{{ old('rent_per_bed_room_month', $property?->fieldValue('rent_per_bed_room_month') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Rent Band (shown live) <span
                                        class="text-red-500">*</span></label>
                                <select required name="rent_band_shown_live"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="Yes" {{ old('rent_band_shown_live', $property?->fieldValue('rent_band_shown_live') ?? '') === 'Yes' ? 'selected' : '' }}>
                                        Yes</option>
                                    <option value="No" {{ old('rent_band_shown_live', $property?->fieldValue('rent_band_shown_live') ?? '') === 'No' ? 'selected' : '' }}>No
                                    </option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Security Deposit <span
                                        class="text-red-500">*</span></label>
                                <select required name="security_deposit_months"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                                    <option value="">— Select —</option>
                                    <option value="1" {{ old('security_deposit_months', $property?->fieldValue('security_deposit_months') ?? '') === '1' ? 'selected' : '' }}>
                                        1</option>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Lock-in / Min Stay (months)
                                </label>
                                <input type="number" step="any" name="lock_in_min_stay_months"
                                    value="{{ old('lock_in_min_stay_months', $property?->fieldValue('lock_in_min_stay_months') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Maintenance / Service Charge
                                </label>
                                <input type="number" step="any" name="maintenance_service_charge"
                                    value="{{ old('maintenance_service_charge', $property?->fieldValue('maintenance_service_charge') ?? '') }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                            </div>
                            <div>
