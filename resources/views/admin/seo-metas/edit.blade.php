...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-0">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
                <h2 class="text-xl sm:text-2xl font-heading text-zendo-navy font-semibold">Edit SEO Meta</h2>
                <p class="text-gray-600 mt-1 text-sm">{{ $pageOptions[$seoMeta->page_key] ?? $seoMeta->page_key }}</p>
            </div>
        ...
        <form action="{{ route('admin.seo-metas.update', $seoMeta) }}" method="POST" class="bg-white rounded-lg shadow-lg p-4 sm:p-6 space-y-5 sm:space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="page_key" value="{{ old('page_key', $seoMeta->page_key) }}" required
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $seoMeta->title) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent"
                    placeholder="Page title for search engines (50-60 characters recommended)" maxlength="255">
                <p class="mt-1 text-sm text-gray-500">
                    <span x-data="{ count: '{{ strlen(old('title', $seoMeta->title ?? '')) }}' }" x-text="$el.closest('div').querySelector('input').value.length + ' characters'"></span>
                </p>
                @error('title')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent"
                    placeholder="Page description for search engines (150-160 characters recommended)" maxlength="500">{{ old('description', $seoMeta->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="keywords" value="{{ old('keywords', $seoMeta->keywords) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent"
            ...
            <div class="pt-4 border-t border-gray-200">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="og_title" value="{{ old('og_title', $seoMeta->og_title) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent"
                    ...
                    <div>
                        ...
                        <textarea name="og_description" rows="2"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent"
                            placeholder="Description shown when shared on social media" maxlength="500">{{ old('og_description', $seoMeta->og_description) }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">OG Image URL</label>
                        <input type="text" name="og_image" value="{{ old('og_image', $seoMeta->og_image) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent"
            ...
            <div class="pt-4 border-t border-gray-200">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="faq_schema" rows="8"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent font-mono text-sm"
                            placeholder='{"@@context":"https://schema.org","@@type":"FAQPage",...}'>{{ old('faq_schema', $seoMeta->faq_schema) }}</textarea>
