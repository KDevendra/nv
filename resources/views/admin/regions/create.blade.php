    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-heading text-zendo-navy font-semibold">Create New Region</h2>
        ...
        <form action="{{ route('admin.regions.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            
            <!-- Region Name -->
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Region Name *</label>
                <input type="text" 
                       name="name" 
                       id="name" 
                       value="{{ old('name') }}"
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          placeholder="Enter region description">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">Inactive regions won't be available for selection</p>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
