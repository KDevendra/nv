...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-2xl font-heading text-zendo-navy font-bold">Regions</h1>
            <p class="text-gray-600 mt-1">Manage geographical regions for user assignment</p>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($regions as $region)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm text-gray-600 font-mono">{{ $region->slug }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    {{ $region->areas_count }} {{ Str::plural('Area', $region->areas_count) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm text-gray-600">{{ $region->sort_order }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <button onclick="toggleStatus({{ $region->id }}, '{{ route('admin.regions.toggle-status', $region) }}')" 
                                        class="status-toggle relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none {{ $region->status ? 'bg-green-500' : 'bg-gray-300' }}">
                                    <span class="inline-block w-4 h-4 transform bg-white rounded-full transition-transform {{ $region->status ? 'translate-x-6' : 'translate-x-1' }}"></span>
                                </button>
                            </td>
...
<script>
function toggleStatus(regionId, url) {
    fetch(url, {
        method: 'PATCH',
        headers: {
