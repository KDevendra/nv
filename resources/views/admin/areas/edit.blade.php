    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-heading text-zendo-navy font-semibold">Edit Area</h2>
        ...
        <form action="{{ route('admin.areas.update', $area) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <select name="region_id" 
                        id="region_id"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-zendo-gold focus:border-transparent @error('region_id') border-red-500 @enderror"
                        required>
                    <option value="">Select a region</option>
                    @foreach($regions as $region)
                        <option value="{{ $region->id }}" {{ old('region_id', $area->region_id) == $region->id ? 'selected' : '' }}>
                            {{ $region->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <!-- Area Name -->
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Area Name *</label>
                <input type="text" 
                       name="name" 
                       id="name" 
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">Inactive areas won't be available for selection</p>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
