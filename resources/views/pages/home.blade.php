         ...
         <section class="relative bg-zendo-navy min-h-screen flex flex-col items-center justify-center pt-20 text-center">
            ...
            <div class="absolute inset-0 z-0">
               ...
               @elseif($heroSection->poster_image)
                  <img src="{{ asset($heroSection->poster_image) }}" alt="{{ $heroSection->title }}"
            ...
            <div class="relative z-10 max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
               <div>
                  <h1 class="text-4xl sm:text-5xl lg:text-7xl font-heading text-white leading-tight animate-on-scroll fade-in-up hero-text-shadow"
                     ...
                     {{ $heroSection->title }} @if($heroSection->highlight_text)<span
                     class="text-zendo-gold">{{ $heroSection->highlight_text }}</span>@endif
                  </h1>
                  ...
                  <div class="flex justify-center flex-wrap gap-2 mb-2">
                     @forelse($serviceTypes as $index => $serviceType)
                        <button type="button"
                           ...
                           data-tab="{{ $serviceType->slug }}" style="border-color: rgb(11, 44, 61);">
                           {{ $serviceType->name }}
                           <div
                              class="tab-pointer absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full w-0 h-0 border-l-8 border-l-transparent border-r-8 border-r-transparent border-t-8 border-t-white {{ $index === 0 ? '' : 'hidden' }}">
                           </div>
                        </button>
                     @empty
                        <!-- Fallback tabs if no service types -->
                        <button type="button"
                           class="search-tab-button group relative px-8 py-3 rounded-t-lg font-highlight bg-gray-200/80 text-gray-700 font-semibold hover:bg-white hover:text-zendo-navy"
                           data-tab="buy" style="border-color: rgb(11, 44, 61);">
                           Buy
                           <div
                              class="tab-pointer absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full w-0 h-0 border-l-8 border-l-transparent border-r-8 border-r-transparent border-t-8 border-t-white hidden">
                           </div>
                        </button>
                        <button type="button"
                           class="search-tab-button group relative px-8 py-3 rounded-t-lg backdrop-blur-sm font-highlight bg-gray-200/80 text-gray-700 font-semibold hover:bg-white hover:text-zendo-navy"
                           data-tab="lease" style="border-color: rgb(11, 44, 61);">
                           Lease
                           <div
                              class="tab-pointer absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full w-0 h-0 border-l-8 border-l-transparent border-r-8 border-r-transparent border-t-8 border-t-white hidden">
                           </div>
                        ...
                        <button type="button"
                           class="search-tab-button group relative px-8 py-3 rounded-t-lg backdrop-blur-sm font-highlight active bg-white text-zendo-navy font-bold"
                           data-tab="rental" style="border-color: rgb(11, 44, 61);">
                           Rental
                           <div
                              class="tab-pointer absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full w-0 h-0 border-l-8 border-l-transparent border-r-8 border-r-transparent border-t-8 border-t-white">
                           </div>
                        </button>
                     @endforelse
                  </div>
                           ...
                           <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                              ...
                              <div class="w-full">
                                 ...
                                 <select id="type" name="type"
                                    class="w-full px-4 py-3 rounded-md border border-gray-300 focus:border-zendo-navy focus:ring-1 focus:ring-zendo-navy text-gray-700 font-body transition-colors">
                                    <option disabled>Select Type</option>

                                    @forelse($propertyTypes as $propertyType)
                                       <option value="{{ $propertyType->slug }}"
                                          data-service-types="{{ implode(',', $propertyType->serviceTypes->pluck('slug')->toArray()) }}"
                                          {{ $propertyType->slug == 'warehouse' ? 'selected' : '' }}>
                                          {{ $propertyType->name }}
                                       </option>
                                    @empty
                                       ...
                                       <option value="warehouse-land">Warehouse Land</option>
                              ...
                              <div class="w-full">
                                 ...
                                 <input type="text" id="location" name="location" placeholder="Enter Location (City / Area)"
                                    class="w-full px-4 py-3 rounded-md border border-gray-300 focus:border-zendo-navy focus:ring-1 focus:ring-zendo-navy text-gray-700 font-body transition-colors"
                                    autocomplete="off">
                              </div>
                              <!-- Button -->
                              <div class="w-full">
                                 <button type="submit"
                                    class="w-full h-[52px] px-8 rounded-full transition-all transform hover:scale-105 shadow-lg font-highlight font-semibold text-lg btn-anim btn-light-bg">
                                    Search
                                 </button>
                              </div>
                           </div>
               ...
               @if ($aboutUs && $aboutUs->checklist_items && count($aboutUs->checklist_items) > 0)
                  @foreach ($aboutUs->checklist_items as $item)
                     <li class="flex items-start">
                        <svg class="flex-shrink-0 w-6 h-6 text-zendo-gold mr-3" xmlns="http://www.w3.org/2000/svg" fill="none"
               ...
               @foreach ($aboutUs->stats as $stat)
                  <div>
                     <p class="text-5xl lg:text-6xl font-medium font-heading text-zendo-gold counter-value"
                        data-target="{{ $stat['value'] ?? '0' }}" @if (isset($stat['prefix']) && !empty($stat['prefix']))
                        data-prefix="{{ $stat['prefix'] }}" @endif @if (isset($stat['suffix']) && !empty($stat['suffix']))
                        data-suffix="{{ $stat['suffix'] }}" @endif @if (strpos($stat['value'] ?? '', '.') !== false)
                        data-decimals="1" @endif>0</p>
      ...
      <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
         ...
         <div class="max-w-4xl mx-auto">
            <form id="inquiryForm" class="grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <!-- Name -->
               <div>
                  <input type="text" name="name" placeholder="Your Full Name" class="inquiry-form-input w-full" required>
               </div>
               <!-- Phone -->
               <div>
                  <input type="tel" name="phone" placeholder="Your Phone Number" class="inquiry-form-input w-full"
                     required>
               </div>
               <!-- Email -->
               <div>
         ...
         <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 card-grid-container">
            ...
            @empty
               ...
               <div class="col-span-full text-center py-12">
                  ...
                  <a href="{{ route('properties.index') }}"
                     class="mt-4 inline-block px-6 py-2 rounded-full bg-zendo-gold text-white font-semibold hover:bg-zendo-navy transition-colors">
                     View All Properties
                  </a>
               </div>
            @endforelse
            ...
            @forelse($whyChooseUsFeatures as $feature)
               ...
               <div class="why-choose-card card-item bg-white rounded-lg shadow-xl p-8 text-center border border-gray-100">
                  <div class="w-20 h-20 mx-auto bg-zendo-light-bg rounded-full flex items-center justify-center mb-6">
                     @if ($feature->icon)
                        <img src="{{ $feature->icon_url }}" alt="{{ $feature->title }} Icon" class="w-10 h-10 text-zendo-gold">
                     @else
                     ...
                     <div class="relative z-10 p-8 md:p-12 lg:p-16">
                        ...
                        @if($videoTour->button_text)
                           <a href="{{ $videoTour->button_link ?? '#' }}"
                              ...
                              <svg class="w-5 h-5 ml-2 transition-transform transform group-hover:translate-x-1" fill="none"
                     ...
                     <div class="relative z-10 p-4 md:p-8 lg:p-0 lg:-mr-12">
                        ...
                        <div class="relative rounded-r-lg overflow-hidden shadow-2xl z-10 w-full h-auto aspect-video">
                           ...
                           @elseif($videoTour->video_path)
                              <video controls class="w-full h-full object-cover rounded-r-lg" @if($videoTour->thumbnail_url)
                              poster="{{ $videoTour->thumbnail_url }}" @endif>
                           ...
                           @else
                              <img src="{{ asset('main/images/properties/morden-villa.png') }}" alt="Video Tour Thumbnail"
                           ...
                           <div class="flex items-center mt-auto">
                              @if ($testimonial->avatar)
                                 <img class="w-12 h-12 rounded-full mr-4 object-cover"
                                    src="{{ Storage::url($testimonial->avatar) }}" alt="{{ $testimonial->name }}">
                              @else
                                 <div class="w-12 h-12 rounded-full mr-4 bg-zendo-gold flex items-center justify-center">
   ...
   <section id="luxRealEstateForm" class="lux-real-section">
      ...
      <div class="lux-real-container">
         ...
         <div class="lux-real-left">
            ...
            <img src="{{ asset('main/images/bg/photo-22.jpg') }}" alt="property" class="lux-left-image">
         </div>
         <!-- RIGHT FORM -->
         <div class="lux-real-form">
            ...
            <form id="luxuryInquiryForm">
               ...
               <div id="luxuryMessage" class="mt-4 hidden">
                  ...
                  <div id="luxuryError" class="hidden bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg">
                     <div class="flex items-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                           <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                  ...
                  @forelse($faqs as $index => $faq)
                     ...
                     <div class="faq-item">
                        <button @click="open = (open === {{ $index + 1 }} ? null : {{ $index + 1 }})"
                           ...
                           <svg class="faq-icon w-6 h-6" :class="{ 'rotate-45': open === {{ $index + 1 }} }" fill="none"
                              stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
         ...
         <div class="bg-white rounded-lg shadow-xl p-8 md:p-12 flex flex-col md:flex-row justify-between items-center">
            ...
            <div class="mt-6 md:mt-0">
               <a href="{{ route('contact') }}"
                  class="px-6 py-3 rounded-full font-highlight font-semibold shadow-lg transition-all transform hover:scale-105 inline-block btn-anim btn-light-bg">Contact
                  us
               </a>
            </div>
         </div>
