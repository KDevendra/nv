...
/**
 * SEZ / EOU / STPI Unit currently has no dedicated Form Request — its owner controller
 ...
 */
class SezEouStpiUnitFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.sez-eou-stpi-unit.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.sez-eou-stpi-unit.store'), [
            'action'              => 'submit',
            ...
            'state'               => 'Maharashtra',
            'customs_bonding_status'    => 'Bonded',
        ]);

        $response->assertRedirect(route('owner.properties.index'));

        $entry = PropertyEntry::where('property_type', 'sez_eou_stpi_unit')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-CS-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.sez-eou-stpi-unit.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'customs_bonding_status'     => 'Bonded',
        ...
        $this->assertSame('Bonded', $custom['customs_bonding_status'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.sez-eou-stpi-unit.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
