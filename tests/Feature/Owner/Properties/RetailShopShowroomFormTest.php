...
/**
 * Retail Shop / Showroom currently has no dedicated Form Request — its owner controller
 ...
 */
class RetailShopShowroomFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.retail-shop-showroom.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.retail-shop-showroom.store'), [
            'action'              => 'submit',
        ...
        $entry = PropertyEntry::where('property_type', 'retail_shop_showroom')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-CR-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.retail-shop-showroom.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'ceiling_height_ft'     => '12',
        ...
        $this->assertSame('12', $custom['ceiling_height_ft'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.retail-shop-showroom.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
