...
/**
 * Builder Floor currently has no dedicated Form Request — its owner controller
 ...
 */
class BuilderFloorFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.builder-floor.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.builder-floor.store'), [
            'action'              => 'submit',
            ...
            'state'               => 'Maharashtra',
            'stilt_covered_parking'    => '1 stilt',
        ]);

        $response->assertRedirect(route('owner.properties.index'));

        $entry = PropertyEntry::where('property_type', 'builder_floor')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-RB-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.builder-floor.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'stilt_covered_parking'     => '1 stilt',
        ...
        $this->assertSame('1 stilt', $custom['stilt_covered_parking'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.builder-floor.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
