...
/**
 * Service Apartment / Co-living / PG currently has no dedicated Form Request — its owner controller
 ...
 */
class ServiceApartmentPgFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.service-apartment-pg.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.service-apartment-pg.store'), [
            'action'              => 'submit',
        ...
        $entry = PropertyEntry::where('property_type', 'service_apartment_pg')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-RS-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.service-apartment-pg.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'gender_policy'     => 'Co-ed',
        ...
        $this->assertSame('Co-ed', $custom['gender_policy'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.service-apartment-pg.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
