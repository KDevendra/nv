...
/**
 * Building / Multi-tenant currently has no dedicated Form Request — its owner controller
 ...
 */
class MultiTenantBuildingFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.multi-tenant-building.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.multi-tenant-building.store'), [
            'action'              => 'submit',
            ...
            'state'               => 'Maharashtra',
            'dg_set_capacity'    => '500 KVA',
        ]);

        $response->assertRedirect(route('owner.properties.index'));

        $entry = PropertyEntry::where('property_type', 'multi_tenant_building')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-CB-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.multi-tenant-building.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'dg_set_capacity'     => '500 KVA',
        ...
        $this->assertSame('500 KVA', $custom['dg_set_capacity'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.multi-tenant-building.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
