...
/**
 * Residential Plot / Land currently has no dedicated Form Request — its owner controller
 ...
 */
class ResidentialPlotLandFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.residential-plot-land.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.residential-plot-land.store'), [
            'action'              => 'submit',
        ...
        $entry = PropertyEntry::where('property_type', 'residential_plot_land')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-RP-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.residential-plot-land.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'plot_shape'     => 'Rectangular',
        ...
        $this->assertSame('Rectangular', $custom['plot_shape'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.residential-plot-land.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
