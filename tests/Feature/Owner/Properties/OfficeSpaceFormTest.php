...
/**
 * Office Space currently has no dedicated Form Request — its owner controller
 ...
 */
class OfficeSpaceFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.office-space.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.office-space.store'), [
            'action'              => 'submit',
            ...
            'state'               => 'Maharashtra',
            'seating_workstation_capacity'    => '40',
        ]);

        $response->assertRedirect(route('owner.properties.index'));

        $entry = PropertyEntry::where('property_type', 'office_space')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-CO-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.office-space.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'seating_workstation_capacity'     => '40',
        ...
        $this->assertSame('40', $custom['seating_workstation_capacity'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.office-space.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
