...
/**
 * Hotel / Resort / Guest House / Banquet currently has no dedicated Form Request — its owner controller
 ...
 */
class HotelResortGuesthouseBanquetFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.hotel-resort-guesthouse-banquet.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.hotel-resort-guesthouse-banquet.store'), [
            'action'              => 'submit',
            ...
            'state'               => 'Maharashtra',
            'banquet_guest_capacity_pax'    => '300',
        ]);

        $response->assertRedirect(route('owner.properties.index'));

        $entry = PropertyEntry::where('property_type', 'hotel_resort_guesthouse_banquet')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-CH-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.hotel-resort-guesthouse-banquet.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'banquet_guest_capacity_pax'     => '300',
        ...
        $this->assertSame('300', $custom['banquet_guest_capacity_pax'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.hotel-resort-guesthouse-banquet.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
