...
/**
 * Commercial / Institutional Land currently has no dedicated Form Request — its owner controller
 ...
 */
class CommercialInstitutionalLandFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.commercial-institutional-land.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.commercial-institutional-land.store'), [
            'action'              => 'submit',
        ...
        $entry = PropertyEntry::where('property_type', 'commercial_institutional_land')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-CL-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.commercial-institutional-land.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'total_land_area'     => '5000',
        ...
        $this->assertSame('5000', $custom['total_land_area'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.commercial-institutional-land.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
