...
/**
 * Agricultural / Farm Land currently has no dedicated Form Request — its owner controller
 ...
 */
class AgriculturalFarmLandFormTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $response = $this->actingAs($this->owner)->get(route('owner.properties.agricultural-farm-land.create'));
    ...
    {
        $response = $this->actingAs($this->owner)->post(route('owner.properties.agricultural-farm-land.store'), [
            'action'              => 'submit',
        ...
        $entry = PropertyEntry::where('property_type', 'agricultural_farm_land')->first();
        $this->assertNotNull($entry);
        $this->assertSame('ZI-CA-001', $entry->code);
    ...
    {
        $this->actingAs($this->owner)->post(route('owner.properties.agricultural-farm-land.store'), [
            'action'               => 'submit',
            'submitter_full_name'  => 'Test Submitter',
            'city'                 => 'Pune',
            'soil_type'     => 'Loamy',
        ...
        $this->assertSame('Loamy', $custom['soil_type'] ?? null);
    }
    ...
    public function update_persists_edited_fields_while_entry_is_a_draft(): void
    {
        ...
        $response = $this->actingAs($this->owner)->put(route('owner.properties.agricultural-farm-land.update', $entry), [
            'action' => 'draft',
            'city'   => 'Bengaluru',
        ]);
