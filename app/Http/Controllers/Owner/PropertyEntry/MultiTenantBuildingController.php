...
class MultiTenantBuildingController extends Controller
{
    ...
    public function create(): View
    {
        ...
        return view('owner.properties.multi-tenant-building.create', compact('slots'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $data = ['property_type' => 'multi_tenant_building', 'field_officer_id' => auth()->id(), 'status' => $status];
        ...
        if (isset($data['property_type']) && $data['property_type'] !== 'multi_tenant_building') {
            $customFields['property_sub_type'] = $data['property_type'];
        }
        $data['property_type'] = 'multi_tenant_building';
        ...
        PropertyEntryLog::create([
            ...
            'note'              => $status === 'draft' ? 'Draft saved' : 'Multi-Tenant Commercial Building submitted',
        ]);
    ...
    public function edit(PropertyEntry $property): View
    {
        abort_if(auth()->user()->role !== 'owner', 403);
        $slots = self::PHOTO_SLOTS;
        return view('owner.properties.multi-tenant-building.create', compact('property', 'slots'));
    }
