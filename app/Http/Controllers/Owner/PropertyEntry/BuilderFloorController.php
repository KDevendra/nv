...
class BuilderFloorController extends Controller
{
    ...
    public function create(): View
    {
        ...
        return view('owner.properties.builder-floor.create', compact('slots'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $data = ['property_type' => 'builder_floor', 'field_officer_id' => auth()->id(), 'status' => $status];
        ...
        if (isset($data['property_type']) && $data['property_type'] !== 'builder_floor') {
            $customFields['property_sub_type'] = $data['property_type'];
        }
        $data['property_type'] = 'builder_floor';
        ...
        PropertyEntryLog::create([
            ...
            'note'              => $status === 'draft' ? 'Draft saved' : 'Builder Floor submitted',
        ]);
    ...
    public function edit(PropertyEntry $property): View
    {
        abort_if(auth()->user()->role !== 'owner', 403);
        $slots = self::PHOTO_SLOTS;
        return view('owner.properties.builder-floor.create', compact('property', 'slots'));
    }
