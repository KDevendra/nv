...
class HotelResortGuesthouseBanquetController extends Controller
{
    ...
    public function create(): View
    {
        ...
        return view('owner.properties.hotel-resort-guesthouse-banquet.create', compact('slots'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $data = ['property_type' => 'hotel_resort_guesthouse_banquet', 'field_officer_id' => auth()->id(), 'status' => $status];
        ...
        if (isset($data['property_type']) && $data['property_type'] !== 'hotel_resort_guesthouse_banquet') {
            $customFields['property_sub_type'] = $data['property_type'];
        }
        $data['property_type'] = 'hotel_resort_guesthouse_banquet';
        ...
        PropertyEntryLog::create([
            ...
            'note'              => $status === 'draft' ? 'Draft saved' : 'Hospitality — Hotel / Resort / Guest House submitted',
        ]);
    ...
    public function edit(PropertyEntry $property): View
    {
        abort_if(auth()->user()->role !== 'owner', 403);
        $slots = self::PHOTO_SLOTS;
        return view('owner.properties.hotel-resort-guesthouse-banquet.create', compact('property', 'slots'));
    }
