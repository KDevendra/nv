...
class OfficeSpaceController extends Controller
{
    ...
    public function create(): View
    {
        ...
        return view('owner.properties.office-space.create', compact('slots'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $data = ['property_type' => 'office_space', 'field_officer_id' => auth()->id(), 'status' => $status];
        ...
        if (isset($data['property_type']) && $data['property_type'] !== 'office_space') {
            $customFields['property_sub_type'] = $data['property_type'];
        }
        $data['property_type'] = 'office_space';
        ...
        PropertyEntryLog::create([
            ...
            'note'              => $status === 'draft' ? 'Draft saved' : 'Commercial Office Space submitted',
        ]);

        $msg = $status === 'draft' ? 'Draft saved successfully.' : 'Commercial Office Space submitted successfully.';
        if ($status === 'draft') {
            return redirect()->route('owner.properties.office-space.edit', $entry)
    ...
    public function edit(PropertyEntry $property): View
    {
        abort_if(auth()->user()->role !== 'owner', 403);
        $slots = self::PHOTO_SLOTS;
        return view('owner.properties.office-space.create', compact('property', 'slots'));
    }
    ...
    public function update(Request $request, PropertyEntry $property): RedirectResponse
    {
        ...
        if ($status === 'draft') {
            return redirect()->route('owner.properties.office-space.edit', $property)
                ->with('success', 'Draft saved successfully.')
                ->with('wizard_step', $request->input('wizard_step', 0));
        }
