...
class RetailShopShowroomController extends Controller
{
    ...
    public function create(): View
    {
        ...
        return view('owner.properties.retail-shop-showroom.create', compact('slots'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $data = ['property_type' => 'retail_shop_showroom', 'field_officer_id' => auth()->id(), 'status' => $status];
        ...
        if (isset($data['property_type']) && $data['property_type'] !== 'retail_shop_showroom') {
            $customFields['property_sub_type'] = $data['property_type'];
        }
        $data['property_type'] = 'retail_shop_showroom';
        ...
        PropertyEntryLog::create([
            ...
            'note'              => $status === 'draft' ? 'Draft saved' : 'Retail Shop / Showroom submitted',
        ]);
    ...
    public function edit(PropertyEntry $property): View
    {
        abort_if(auth()->user()->role !== 'owner', 403);
        $slots = self::PHOTO_SLOTS;
        return view('owner.properties.retail-shop-showroom.create', compact('property', 'slots'));
    }
