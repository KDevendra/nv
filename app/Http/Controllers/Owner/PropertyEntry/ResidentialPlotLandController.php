...
class ResidentialPlotLandController extends Controller
{
    ...
    public function create(): View
    {
        ...
        return view('owner.properties.residential-plot-land.create', compact('slots'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $data = ['property_type' => 'residential_plot_land', 'field_officer_id' => auth()->id(), 'status' => $status];
        ...
        if (isset($data['property_type']) && $data['property_type'] !== 'residential_plot_land') {
            $customFields['property_sub_type'] = $data['property_type'];
        }
        $data['property_type'] = 'residential_plot_land';
        ...
        PropertyEntryLog::create([
            ...
            'note'              => $status === 'draft' ? 'Draft saved' : 'Residential Plot / Land submitted',
        ]);
    ...
    public function edit(PropertyEntry $property): View
    {
        abort_if(auth()->user()->role !== 'owner', 403);
        $slots = self::PHOTO_SLOTS;
        return view('owner.properties.residential-plot-land.create', compact('property', 'slots'));
    }
